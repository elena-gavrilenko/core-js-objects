...
function makeWord(lettersObject) {
  const values = Object.values(lettersObject).flat().sort();
  return values
    .map((el) =>
      Object.keys(lettersObject).find((k) => lettersObject[k].includes(el))
    )
    .join('');
}
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] > 25 && sum(queue.slice(0, i)) < queue[i] - 25) {
      isSell = false;
      break;
    }
  }
